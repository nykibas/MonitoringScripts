...
# ==============================================================================
# NOM DU SCRIPT : full_db_rman_backup.sh
# PROPRIÉTAIRE  : Yannick Nzau
# INSTANCE      : SBCPROD
# SERVEUR       : SKN010PRODSRVCBSBDD12
...
export ORACLE_SID=SBCPROD
export ORACLE_HOME=/u01/app/oracle/product/19.0.0/dbhome_1 
export PATH=$ORACLE_HOME/bin:$PATH
export DATE_STR=$(date +%Y%m%d_%H%M)

# 2. Définition des répertoires et du fichier log
BACKUP_ROOT="/backup/rman"
DB_DUMP_DIR="$BACKUP_ROOT/FULL_BD_DUMP"
ARCH_DUMP_DIR="$BACKUP_ROOT/ARCHIVELOGS_DUMP"
LOG_FILE="$BACKUP_ROOT/logs/rman_backup_parallel_$DATE_STR.log"
...
RUN {
  ...
  # 3 & 5. Backup Full DB + Controlfile + SPFILE (Dossier FULL_BD_DUMP)
  BACKUP AS COMPRESSED BACKUPSET DATABASE 
  FORMAT '$DB_DUMP_DIR/DB_FULL_%d_%T_%U.bak'
  TAG 'FULL_DB_PARALLEL'
